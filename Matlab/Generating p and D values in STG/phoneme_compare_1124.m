...
IndexC = strfind(conditions(:,1), 'GA');
IndexC = find(not(cellfun('isempty', IndexC)));
...
IndexC = strfind(conditions(:,2), 'GA');
IndexC = find(not(cellfun('isempty', IndexC)));
OUTPUT.stim(temp(IndexC),2) = OUTPUT.stim(temp(IndexC),2)+300;
 

OUTPUT.Conditions = {'Aud','Cong','Incong','Vis-Alone','FaceOnset','A_BA','C_BA','I_BA','V_BA','A_DA','C_DA','I_DA','V_DA','A_GA','C_GA','I_GA','V_GA','A_THA','C_THA','I_THA','V_THA'};
