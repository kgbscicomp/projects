function [dim_w,dim_h] = ECOG_grid_maker_v2(CurrGridName)
...
dim_direction = [];
while isempty(dim_direction)
temp_dir = input(['Are the first electrode numbers (contacts 1,2,3,etc.) in the same row (enter 0) or column (enter 1)?: ']);
if ismember(temp_dir,[ 0 1])
    dim_direction = temp_dir;
end
end
...
% For 5 x 2, [1 5 6 10];
clicknumbers = [];
if dim_direction == 0 % same row
    for j=[1 dim_h]
        clicknumbers = [clicknumbers (((j-1)*dim_w)+1)];
        clicknumbers = [clicknumbers (((j-1)*dim_w)+dim_w)];
    end
elseif dim_direction == 1 % same column
   for j=[1 dim_w]
    clicknumbers = [clicknumbers (((j-1)*dim_h)+1)];
    clicknumbers = [clicknumbers (((j-1)*dim_h)+dim_h)];
   end
end
...
f.Name = 'Click the corners of the grid/depth probe from smallest to largest contact';

disp('************************************************************************');
disp('************************************************************************');
disp('*********** Click the 4 corners of the grid or 2 ends of the ***********');
disp('******** depth probe from smallest to largest electrode contact ********');
disp('************************************************************************');
disp('************************************************************************');
