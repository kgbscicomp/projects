function ECOG_filter_analysis_PLF(subject,task,ALLEEG, EEG,CURRENTSET, Conditions,CondNums,filter_range,epoch,baseline,input_bad_chans,ButterRp,ButterRs)
...
for el=1:size(data,2)
    ...
    for yy = 1:size(hband_filters,1)
        ...
        hband_sig_data(:,yy) = angle(hilbert(hband_sig_data(:,yy)));
    end    
