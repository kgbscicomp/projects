...
for j=1:NumConds
    ...
    AVG.(genvarname(char(strcat('I',num2str(plot_conds(j)))))) = nan(MaxChans,num_frex,trial_length,MaxTrials);
    AVG.(genvarname(char(strcat('I',num2str(plot_conds(j)),'_err')))) = nan(MaxChans,trial_length);
    AVG.(genvarname(char(strcat('I',num2str(plot_conds(j)),'_avg')))) = nan(MaxChans,trial_length);
...
for chanx = plot_chans %[29 30 37 38 43:46 86:91]
    ...
    for j=1:NumConds
        ...
        PLOT.(genvarname(char(strcat('ERP_I',num2str(plot_conds(j)))))) = [];
        PLOT.(genvarname(char(strcat('C',num2str(plot_conds(j)))))) = [];
        PLOT.(genvarname(char(strcat('I',num2str(plot_conds(j)))))) = [];
    end
    ...
    for j=1:NumConds
        ...
        for i=1:NTrials(j)
            PLOT.(genvarname(char(strcat('ERP_I',num2str(plot_conds(j))))))(:,i) = PLOT.(genvarname(char(strcat('ERP',num2str(plot_conds(j))))))(:,i)-AVG.(genvarname(char(strcat('ERP',num2str(plot_conds(j)),'_avg'))))(chanx,:)';
        end
    end
...
%             disp(['Perm: ' int2str(k) ' of ' int2str(NPerm)])
    ...
    % Power calculated on Induced (ERP trials minus average ERP)
    for j=1:NumConds
        temp = PLOT.(genvarname(char(strcat('ERP_I',num2str(plot_conds(j)))))); % time x trials format
        EEGpnts = size(temp,1);
        EEGtrials = size(temp,2);
        ...
        for fi=1:num_frex
            ...
            sig1 = reshape(convolution_result_fft,EEGpnts,EEGtrials);
            power1 = sig1.*conj(sig1); % same as mean(abs(sig1).^2,2);
            PLOT.(genvarname(char(strcat('I',num2str(plot_conds(j))))))(fi,:,:) = power1;
        end
    end
    ...
    for fi=1:num_frex
        for j=1:NumConds
            tempFreqData = [tempFreqData squeeze(PLOT.(genvarname(char(strcat('I',num2str(plot_conds(j))))))(fi,:,:))];
        end
        STDthresh(fi) = std(std(tempFreqData(zbaseline(1):xaxis(end),:)))*20;
    end
    ...
    for j=1:NumConds
        C_bad_trials = [];
        for fi=1:num_frex
            power1 = squeeze(PLOT.(genvarname(char(strcat('I',num2str(plot_conds(j))))))(fi,:,:));
            stdDAT = std(power1(zbaseline(1):xaxis(end),:));
            for i=1:length(stdDAT)
                if stdDAT(i)>STDthresh(fi)
                C_bad_trials(fi,i) = 1;
        ...
        tempDat = [];
        for fi=1:num_frex
            power1 = squeeze(PLOT.(genvarname(char(strcat('I',num2str(plot_conds(j))))))(fi,:,C_good_trials));
            meanDAT = mean(power1,2)';
            meanC = mean(meanDAT(zbaseline));
            stdx = std(meanDAT(zbaseline));
        ...
        AVG.(genvarname(char(strcat('I',num2str(plot_conds(j)),'_avg'))))(chanx,:) = mean(tempDat,1);
        AVG.(genvarname(char(strcat('I',num2str(plot_conds(j))))))(chanx,:,:,1:NTrials(j)) = PLOT.(genvarname(char(strcat('I',num2str(plot_conds(j))))));
        ...
        for k=1:NPerm
            ...
            for fi=1:num_frex % calculate power for each freq
                % pull the rand freqs many times then repeat the avging above
                meanDAT = mean(PLOT.(genvarname(char(strcat('I',num2str(plot_conds(j))))))(fi,:,permX),3);
                meanC = mean(meanDAT(zbaseline));
                stdx = std(meanDAT(zbaseline));
                if db_on==1
                    tempDB = [10*log10(meanDAT/meanC)]';
...
%             disp(['Perm: ' int2str(k) ' of ' int2str(NPerm)])
        ...
        AVG.(genvarname(char(strcat('I',num2str(plot_conds(j)),'_err'))))(chanx,:) = std(tempPerm);
    end
    counter = counter+1;
end
if SPECS.Save==1
...
for chanx = plot_chans %[29 30 37 38 43:46 86:91]
    ...
    zerobarI = zeros(1,trial_length);
    zerobarI(zerotime) = min(AVG.(genvarname(char(strcat('I',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis));
    zerobarI = zerobarI(xaxis);
...
% % % % %     end
    ...
    hold on
    shadedErrorBar(1:length(xaxis),AVG.(genvarname(char(strcat('C',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis),AVG.(genvarname(char(strcat('C',num2str(plot_conds(1)),'_err'))))(chanx,xaxis),colorarray(1),.5);
    if NumConds>1
        for j=2:NumConds
...
%     suptitle(strcat('CHANNEL:   ', chan_names(chanx)))
    ...
    subplot(1,3,3)
    h(1) = plot(AVG.(genvarname(char(strcat('I',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(1)));
    hold on
    shadedErrorBar(1:length(xaxis),AVG.(genvarname(char(strcat('I',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis),AVG.(genvarname(char(strcat('I',num2str(plot_conds(1)),'_err'))))(chanx,xaxis),colorarray(1),.5);
    if NumConds>1
        for j=2:NumConds
            h(j) = plot(AVG.(genvarname(char(strcat('I',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(j)));
            shadedErrorBar(1:length(xaxis),AVG.(genvarname(char(strcat('I',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),AVG.(genvarname(char(strcat('I',num2str(plot_conds(j)),'_err'))))(chanx,xaxis),colorarray(j),.5);
        end
    end
    plot(zerobarI)
    hold off
