...
if sum(strcmp(PLOT.types,'ERSP')>0),AVG.ERSPstats= nan(MaxChans,trial_length);end
if sum(strcmp(PLOT.types,'Induced')>0),AVG.Inducedstats= nan(MaxChans,trial_length);end
    ...
    if SPECS.Options.ERSP==1
        ...
        for ERSPcLength=1:length(ERSPcond)
            ...
            for j=1:NumConds
                ...
                if sum(strcmp(PLOT.types,'ERSP-individ')>0),AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(j)),'_indiv'))))(chanx,:,1:NTrials_ERSP(j)) = mean(tempDatIndiv,3)';end
                if sum(strcmp(PLOT.types,'ERSP')>0),AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(j)),'_spect'))))(chanx,:,:) = tempDat;end
            ...
            elseif NumConds==3 % ANOVA
                ...
                for timex=1:plotSize(2)
                    if Analysis.PermTest(timex)< .05
                        AVG.ERSPstats(chanx,timex)=1;
                    else
                        AVG.ERSPstats(chanx,timex)=0;
                    end
                end
