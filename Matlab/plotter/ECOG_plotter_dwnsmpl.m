...
for chanx = plot_chans %[29 30 37 38 43:46 86:91]
    ...
    zerobarInduced = zeros(1,size(PLOT.C1,1));
    zerobarERP = zeros(1,size(PLOT.C1,1));
    zerobarInduced(zerotime) = min(mean(PLOT.C1,2)); % 0 ms from aud onset
    zerobarERP(zerotime) = min(mean(PLOT.ERP1,2))/2; % 0 ms from aud onset
    
    figure(5)
    shadedErrorBar(1:size(PLOT.C1,1),mean(PLOT.C1,2)',PLOT.C1_err,colorarray(1),.5);
    hold on
    ...
    figure(6)
    shadedErrorBar(1:size(PLOT.ERP1,1),mean(PLOT.ERP1,2)',PLOT.ERP1_err,colorarray(1),.5);
    hold on
