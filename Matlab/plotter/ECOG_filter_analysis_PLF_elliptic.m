function ECOG_filter_analysis_PLF_elliptic(subject,task,ALLEEG, EEG,CURRENTSET, Conditions,CondNums,filter_range,epoch,baseline,input_bad_chans,ButterRp,ButterRs)
...
% % % % % save('temp.mat'); %(strcat(subject,'_',task));
% % % % % pause();
...
hband_elliptic_wide=zeros(size(data,1),size(hband_filters,1),size(data,2));
eband_sig_data=zeros([size(data,1) size(hband_filters,1)]);
tempEEG = EEG;
for el=1:size(data,2)
    ...
    for yy = 1:size(hband_filters,1)
        ...
        eband_sig_data(:,yy) = elliptic_eeglabdata(data(:,el),hband,srate,ButterRp,ButterRs);%3,60);
        eband_sig_data(:,yy) = (hilbert(eband_sig_data(:,yy)));
        hband_elliptic_wide(:,yy,el)=mean(eband_sig_data,2);
    end    
    hband_sig_data=zeros([size(data,1) size(hband_filters,1)]);
    eband_sig_data=zeros([size(data,1) size(hband_filters,1)]);
end
clear hband_sig_data
clear eband_sig_data
...
for elec=1:num_el
    ...
    for xxx=1:size(hband_filters,1)
        for yyy=1:numevents
            ...
            hband_sig_elliptic_wide(yyy,:,xxx,elec)=...
                squeeze(hband_elliptic_wide(currsample+(epoch_adj(1)*srate_new):currsample+(epoch_adj(2)*srate_new),xxx,elec))';
        end
    end
end
