...
%     var_arry = zeros(1,size(tempdata,2));
%     for i=1:size(tempdata,2)
%         var_arry(i) = var(tempdata(:,i));
%     end
...
%     disp(['New ref is ' char(chan_names( Ref_electrode))])
    for i=1:size(data,2)
        data(:,i) = data(:,i)-data(:,Ref_electrode);
    end
    
    data(:,Ref_electrode)=[];
    chan_lbls([Ref_electrode])=[];
...
while RejThreshNew>0
    ...
    while isempty(b) && isempty(c) && ~isempty(bad_chans_temp)
        if ismember(reference,[0 2:3])
            if ismember(reference,2:3) % laplacian or car by grid
                chan_names = {};
                for i=1:length(chan_lbls)
                    temp = char(EEG.chanlocs(1,chan_lbls(i)).labels);
                    chan_names{i} = temp;
        ...
        elseif ismember(reference,1) % lowest variance
            a=var(tempdata);
            b=find(a>(RejThreshNew*median(a)));
            c=[];
