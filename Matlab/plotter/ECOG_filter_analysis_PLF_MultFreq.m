function ECOG_filter_analysis_PLF_MultFreq(subject,task,ALLEEG, EEG,CURRENTSET, Conditions,CondNums,filter_range,filter_width,epoch,baseline,input_bad_chans,analysis_ch,ButterRp,ButterRs)
...
data2 = zeros(size(data,1),length(analysis_ch));
for i=1:length(analysis_ch)
    temp = find(chan_lbls==analysis_ch(i));
    data2(:,i) = data(:,temp);
    chan_names_new(i) = chan_names(temp);
end
data = data2;
clear data2
...
hband_filters =[];
hband_filters(:,1) = [filter_range(1):filter_width:filter_range(2)-filter_width+1]';
hband_filters(:,2) = [filter_range(1)+filter_width-1:filter_width:filter_range(2)]';
bad_freqs = [];
for i=51:69
    bad_freqs(1,size(bad_freqs,2)+1:size(bad_freqs,2)+size(i:60:2000,2)) = i:60:2000;
end
bad_freqs_rm =[];
for i=1:size(hband_filters,1)
    for j=hband_filters(i,1):hband_filters(i,2)
        if ismember(j,bad_freqs)
            bad_freqs_rm = [bad_freqs_rm i];
        end
    end
end
hband_filters(bad_freqs_rm,:)=[];
...
mband_morlet_wide = zeros(size(data,1),size(hband_filters,1),size(data,2));
tempEEG = EEG;
for el=1:size(data,2)
    ...
    for yy = 1:size(hband_filters,1)
        ...
        bb = bbpower_phase(data(:,el),srate,[hband_filters(yy,1) hband_filters(yy,2)],filter_width,30,false); %30
        mband_morlet_wide(:,yy,el)=bb;
    end    
    hband_sig_data=zeros([size(data,1) size(hband_filters,1)]);
end
...
for elec=1:num_el
    ...
    for xxx=1:size(hband_filters,1)
        for yyy=1:numevents
            ...
            mband_sig_morlet_wide(yyy,:,xxx,elec)=...
                squeeze(mband_morlet_wide(currsample+(epoch_adj(1)*srate_new):currsample+(epoch_adj(2)*srate_new),xxx,elec))';
        end
    end
end
...
save(strcat(subject,'_',task),'-v7.3', '-regexp', '^(?!(ALLCOM|ALLEEG|ALLERP|ALLERPCOM|CURRENTERP|CURRENTSET|CURRENTSTUDY|EEG|ERP|LASTCOM|PLUGINLIST|STUDY|eeglabUpdater|plotset|tempEEG)$).');
end
