...
for chanx = plot_chans %[29 30 37 38 43:46 86:91]
    ...
    while analysescounter <= length(PLOT.types)
        ...
        if sum(strcmp(PLOT.types(analysescounter),'ITPC-line'))>0
            subplot(PLOT.rowcount,PLOT.colcount,PLOT.plotlocs(analysescounter,:))
            if ismember(NumConds,2)
                h(1) = plot(AVG.(genvarname(char(strcat('ITPC',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(1)));
                hold on
                ...
                j=2;
                h(j) = plot(AVG.(genvarname(char(strcat('ITPC',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(j)));
            ...
            if ismember(NumConds,[1,3:4])
                bar((AVG.ITPCstats(chanx,xaxis)*min(AVG.(genvarname(char(strcat('ITPC',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis))),'FaceColor',[.8 .8 .8]);
                hold on
                for j=1:NumConds
                    h(j) = plot(AVG.(genvarname(char(strcat('ITPC',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(j)));
        ...
        if sum(strcmp(PLOT.types(analysescounter),'R2-line'))>0
            subplot(PLOT.rowcount,PLOT.colcount,PLOT.plotlocs(analysescounter,:))
            if ismember(NumConds,2)
                h(1) = plot(AVG.(genvarname(char(strcat('R2',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(1)));
                hold on
                ...
                j=2;
                h(j) = plot(AVG.(genvarname(char(strcat('R2',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(j)));
                shadedErrorBar(1:length(xaxis),AVG.(genvarname(char(strcat('R2',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),errbar,colorarray(j),.5);
            end
            if ismember(NumConds,[1,3:4])
                bar((AVG.R2stats(chanx,xaxis)*min(AVG.(genvarname(char(strcat('R2',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis))),'FaceColor',[.8 .8 .8]);
                hold on
                for j=1:NumConds
                    h(j) = plot(AVG.(genvarname(char(strcat('R2',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(j)));
