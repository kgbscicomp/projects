function ECOG_filter_analysis_elliptic(subject,task,ALLEEG, EEG,CURRENTSET, Conditions,CondNums,filter_range,epoch,baseline,srate_downsample,input_bad_chans,ButterRp,ButterRs, ZscoreOn)
...
for el=1:size(data,2)
    ...
    for yy = 1:size(hband_filters,1)
        ...
        hband_sig_data(:,yy) = elliptic_eeglabdata(data(:,el),hband,srate,ButterRp,ButterRs);%3,60);
