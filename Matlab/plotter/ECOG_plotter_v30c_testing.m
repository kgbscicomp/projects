% % % % % function ECOG_plotter_v30c_testing
%% Example
% tic
% SPECS.plot_chans =[1]';
% SPECS.PLOTaxis = [-1 1];
...
% SPECS.Powerbaseline = [];
% SPECS.Freq = [2 20];
% SPECS.FreqWidth =2; % Distance in frequencies between wavelet center frequencies
% SPECS.FreqSpacing = 0; % 0 = Linear spacing. 1 = Log spacing.
% SPECS.FreqCycles = [3 12];%[3]; Will always do a min of 3 cycles
% SPECS.Normalization =2; % 0 = No Normalization, 1 = dB change relative to baseline, 2 = 1/f norm, 
% SPECS.StatsType = 1; % 1 = Parametric. 2 = Non-Parametric. Parametric only works on No-Norm or Whitened Data
% SPECS.NPerm=500; % Num permutations when needed
...
% SPECS.MultCompType = 2; % 0 = None, 1 = FDR, 2 = MaxStat, 3 = Cluster, 4 = Bonferroni correction, 5 = Mean time range
% SPECS.Analysis_Windows = [0 .2]; % SPECS.Analysis_Windows = [.2 .3;.5 .6]; If empty, use full xaxis time-range. Otherwise perform MultCompType within limited time range. Req for MultCompType=5.
...
if SPECS.Replot==0
    ...
    SPECS.Options.types = {'ERP','ERSP-line','ERSP','ERSP-individ','Induced-line','Induced','Induced-individ','ITPC-line','ITPC','R2-line','R2','Distribution','RawSpect-line','RawSpect','ITPA'};
    SPECS.Options.plots = [1 1 .5 1 1 .5 1 1 .5 1 .5 1 1 .5 .5]; % how many of two rows will each plot take in the subplot
    SPECS.Options.ylabel = {'microVolts','Normalized Amplitude','Normalized Amplitude','Normalized Amplitude','Normalized Amplitude','Normalized Amplitude','Normalized Amplitude','Phase-Consistency (0-1)','Phase-Consistency (0-1)','Phase-Consistency (0-1)','Phase-Consistency (0-1)','Mean Normalized Amplitude','Normalized Amplitude','Normalized Amplitude','Phase-Consistency (0-1)'};
        ...
        if SPECS.Options.ERSP==1
            ...
            for ERSPcLength=1:length(ERSP_and_Induced_Array)
                ...
                for j=1:NumConds
                    ...
                    % pull current 1/f
                    oneoverf = OUTPUT.oneoverf(chanx,:);
...
%                 plot(oneoverf_mean)
                ...
                for j=1:NumConds
                    ...
                    [tempDatIndiv,tempDat,tempDatSpectral]=ECOG_plotter_dep_normalize_v8(PLOT.(genvarname(char(strcat(ERSP_and_Induced_Array(ERSPcLength),num2str(plot_conds(j)))))),zbaseline,xaxis,SPECS.Normalization,curr_plot_type,oneoverf_mean,ERSP_and_Induced_Array{ERSPcLength});
                ...
                for PHASEtype = 1:length(PHASEcond)
                    ...
                    if strcmp(PHASEcond(PHASEtype),'ITPC')
                        ...
                        if ~SPECS.MultCompType==3 || NPerm <=200
                            [STATS,Output] = ECOG_plotter_dep_parametric_statistics_ITPC_v4i(input_data,PHASEcond(PHASEtype),PThresh,PTail,SPECS.ErrBars,SPECS.MultCompType,NPerm,zbaseline,line_or_spect,analysis_windows);
                        else
                            ...
                            [STATS,Output] = ECOG_plotter_dep_parametric_statistics_ITPC_v4i(input_data,PHASEcond(PHASEtype),PThresh_temp,PTail,SPECS.ErrBars,SPECS.MultCompType,NPerm_temp,zbaseline,line_or_spect,analysis_windows);
