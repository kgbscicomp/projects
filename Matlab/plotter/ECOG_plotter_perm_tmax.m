...
for chanx = plot_chans %[29 30 37 38 43:46 86:91]
    ...
    test_smpl = round(test_window(1)*srate):round(test_window(2)*srate);
    tempdataA = PLOT.(genvarname(char(strcat('C',num2str(plot_conds(1))))));
    tempdataB = PLOT.(genvarname(char(strcat('C',num2str(plot_conds(2))))));
    [H,P,CI,STATS] = ttest2(tempdataA(test_smpl,:)',tempdataB(test_smpl,:)');
    PLOT.realT_C = STATS.tstat;
    ...
    [H,P,CI,STATS] = ttest2(tempdataA(test_smpl,:)',tempdataB(test_smpl,:)');
    PLOT.realT_ERP = STATS.tstat;
    
    % permutation testing between 2 conditions
    
    RealCombined_ERP = [PLOT.(genvarname(char(strcat('ERP',num2str(plot_conds(1)))))),PLOT.(genvarname(char(strcat('ERP',num2str(plot_conds(2))))))];
    
    PermA_ERP = zeros(size(PLOT.(genvarname(char(strcat('ERP',num2str(plot_conds(1))))))));
    PermB_ERP = zeros(size(PLOT.(genvarname(char(strcat('ERP',num2str(plot_conds(2))))))));
    PLOT.PermResultsERP = zeros(NumPerms,1);
    
    PermA_C = zeros(size(PLOT.(genvarname(char(strcat('C',num2str(plot_conds(1))))))));
    PermB_C = zeros(size(PLOT.(genvarname(char(strcat('C',num2str(plot_conds(2))))))));
    PLOT.PermResultsInduced = zeros(NumPerms,1);
    ...
    % need to run ttest on each perm then calc 95% of max(abs(tstats))
    % test can be done either for timepoints or on window for x chans
    ...
    for zzz=1:NumPerms
        ...
        test1 = RealCombined_ERP(:,LabelRand(1:NumCondA));
        test2 = RealCombined_ERP(:,LabelRand(NumCondA+1:NumCondA+NumCondB));
