...
SPECS.Options.types = {'ERP','ERSP-line','ERSP','ERSP-individ','Induced-line','Induced','ITC-line','ITC','R2-line','R2'};
SPECS.Options.plots = [1 1 .5 1 1 .5 1 .5 1 .5]; % how many of two rows will each plot take in the subplot
...
%    
%   counter=1;  
% while counter<=numanalyses
%     
% end 
%     
%     if temp == .5 && round(temp*length(plot_conds))==temp*length(plot_conds)
%         % put these two plots in the top and bottom panels
%         PLOT.plotnum = PLOT.plotnum+temp*length(plot_conds);
%     elseif temp == .5 && i<numanalyses
%             temp2 = SPECS.Options.plots(find(strcmp(PLOT.types(counter+1),SPECS.Options.types)==1));
%                  
%             end
%         end
%     end
...
%     
%     if temp == .5
%         PLOT.plotnum = PLOT.plotnum*length(plot_conds)*.5;
%     else
%         PLOT.plotnum = PLOT.plotnum*length(plot_conds)*.5
%         
%     rowcount = rowcount+1;
...
%     
%  
% if numanalyses >= 4, numanalyses = [2 ceil(numanalyses/2)];else numanalyses = [1 numanalyses]; end
%     
%     
%     
...
for j=1:NumConds
    ...
    if SPECS.Options.ERSP==1
        ...
        AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(j)),'_spect')))) = nan(MaxChans,num_frex,trial_length);
        AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(j)),'_indiv')))) = nan(MaxChans,trial_length,MaxTrials_ERSP);
    end
end
...
%                 power1 = power1 - repmat(mean(power1(zbaseline,:),1),size(power1,1),1);
                PLOT.(genvarname(char(strcat('ERSP',num2str(plot_conds(j))))))(fi,:,:) = power1;
            end
        end
        ...
        for fi=1:num_frex
            for j=1:NumConds
                tempFreqData = [tempFreqData squeeze(PLOT.(genvarname(char(strcat('ERSP',num2str(plot_conds(j))))))(fi,:,:))];
            end
        ...
        for j=1:NumConds
            C_bad_trials = [];
            for fi=1:num_frex
                power1 = squeeze(PLOT.(genvarname(char(strcat('ERSP',num2str(plot_conds(j))))))(fi,:,:));
            ...
            tempDat = [];
            for fi=1:num_frex
                power1 = squeeze(PLOT.(genvarname(char(strcat('ERSP',num2str(plot_conds(j))))))(fi,:,C_good_trials));
            ...
            AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(j)),'_avg'))))(chanx,:) = mean(tempDat,1);
            AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(j))))))(chanx,:,:,1:NTrials_ERSP(j)) = PLOT.(genvarname(char(strcat('ERSP',num2str(plot_conds(j))))));
            AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(j)),'_indiv'))))(chanx,:,1:NTrials_ERSP(j)) = mean(tempDatIndiv,3)';
            AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(j)),'_spect'))))(chanx,:,:) = tempDat;
            
            if sum(strcmp(PLOT.types,'ERSP-line'))>0
                ...
                if ismember(NumConds,[1 3:4])
                    ...
                    for k=1:NPerm
                        ...
                        for fi=1:num_frex % calculate power for each freq
                            ...
                            meanDAT = mean(PLOT.(genvarname(char(strcat('ERSP',num2str(plot_conds(j))))))(fi,:,permX),3);
                            meanC = mean(meanDAT(zbaseline));
                            stdx = std(meanDAT(zbaseline));
                            if db_on==1
                                tempDB = [10*log10(meanDAT/meanC)]';
                    ...
                    AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(j)),'_err'))))(chanx,:) = std(tempPerm);
                end
            end
        end
        ...
        elseif NumConds==2 % ttest
            ...
            tempdata1 = PLOT.(genvarname(char(strcat('ERSP',num2str(plot_conds(1))))));
            tempdata2 = PLOT.(genvarname(char(strcat('ERSP',num2str(plot_conds(2))))));
            diff = AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(1)),'_avg'))))(chanx,:)-AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(2)),'_avg'))))(chanx,:);
            plotSize = size(diff);
        ...
        elseif NumConds==3 % ANOVA
            ...
            tempdata1 = PLOT.(genvarname(char(strcat('ERSP',num2str(plot_conds(1))))));
            tempdata2 = PLOT.(genvarname(char(strcat('ERSP',num2str(plot_conds(2))))));
            tempdata3 = PLOT.(genvarname(char(strcat('ERSP',num2str(plot_conds(3))))));
            tempMean1 = AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(1)),'_avg'))))(chanx,:);
            tempMean2 = AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(2)),'_avg'))))(chanx,:);
            tempMean3 = AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(3)),'_avg'))))(chanx,:);
        ...
        elseif NumConds==4 % ANOVA
            ...
            tempdata2 = PLOT.(genvarname(char(strcat('ERSP',num2str(plot_conds(2))))));
            tempdata3 = PLOT.(genvarname(char(strcat('ERSP',num2str(plot_conds(3))))));
            tempdata4 = PLOT.(genvarname(char(strcat('ERSP',num2str(plot_conds(4))))));
            tempMean1 = AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(1)),'_avg'))))(chanx,:);
            tempMean2 = AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(2)),'_avg'))))(chanx,:);
            tempMean3 = AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(3)),'_avg'))))(chanx,:);
            tempMean4 = AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(4)),'_avg'))))(chanx,:);
...
for chanx = plot_chans %[29 30 37 38 43:46 86:91]
    if SPECS.Options.ERSP==1
        ...
        zerobarC(zerotime) = min(AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis));
    ...
    while analysescounter <= length(PLOT.types)
        ...
        if sum(strcmp(PLOT.types(analysescounter),'ERSP-line'))>0
            ...
            if ismember(NumConds,[1,3:4])
                bar((AVG.Cstats(chanx,xaxis)*min(AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis))),'FaceColor',[.8 .8 .8]);
                hold on
                for j=1:NumConds
                    h(j) = plot(AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(j)));
