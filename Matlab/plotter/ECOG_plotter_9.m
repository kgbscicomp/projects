...
PLOT.types = SPECS.types;
SPECS.Options.ERP = 1; % All processes need ERP Preprocessing
if max(find(strcmp(SPECS.Options.types,SPECS.types)==1))>1, SPECS.Options.ERSP = 1;else SPECS.Options.ERSP=0; end
...
for j=1:NumConds
    ...
    AVG.(genvarname(char(strcat('C',num2str(plot_conds(j)))))) = nan(MaxChans,num_frex,trial_length,MaxTrials_ERSP);
...
for chanx = plot_chans %[29 30 37 38 43:46 86:91]
    ...
    for j=1:NumConds
        ...
        AVG.(genvarname(char(strcat('C',num2str(plot_conds(j))))))(chanx,:,:,1:NTrials_ERSP(j)) = PLOT.(genvarname(char(strcat('C',num2str(plot_conds(j))))));
