...
% SPECS.FigSize = 1; %1 for full screen, 0 for normal
% SPECS.types = {'ERP','ERSP'};% SPECS.types = {'ERP','ERSP-line','ERSP','ERSP-individ','Induced-line','Induced','Induced-individ','ITPC-line','ITPC','R2-line','R2'};
% ECOG_plotter_15
...
SPECS.Options.types = {'ERP','ERSP-line','ERSP','ERSP-individ','Induced-line','Induced','Induced-individ','ITPC-line','ITPC','R2-line','R2'};
...
bad_trials = SPECS.bad_trials; %edited by JZ 3.17.2015
    ...
    if SPECS.Options.ERSP==1
        ...
        for ERSPcLength=1:length(ERSPcond)
            ...
            for fi=1:num_frex
                ...
                STDthresh(fi) = std(std(tempFreqData(zbaseline(1):xaxis(end),:)))*RejThreshERSP;
            end
            ...
            if sum(strcmp(PLOT.types,strcat(ERSPcond(ERSPcLength),'-line'))>0)
                ...
                if NumConds==1 % 1-PVal confidence interval
                    ...
                    % % % % %
                            plotSize = size(PLOT.(genvarname(char(strcat(ERSPcond(ERSPcLength),num2str(plot_conds(1)))))));
                            ...
                            for k=1:NPerm
                                ...
                                signx( signx==2 )=-1;
                                tempPerm = PLOT.(genvarname(char(strcat(ERSPcond(ERSPcLength),num2str(plot_conds(1)))))).*signx;
                                tempDat = [];
                                for fi=1:num_frex % calculate power for each freq
        ...
        for PHASEcLength=1:length(PHASEcond)
            ...
            for fi=1:num_frex
                ...
                STDthresh(fi) = std(std(tempFreqData(zbaseline(1):xaxis(end),:)))*10000;
            end
            ...
            for j=1:NumConds
                ...
                tempITPC = [];
                for fi=1:num_frex
                    angle1 = squeeze(PLOT.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(j))))))(fi,:,C_good_trials));
                ...
                AVG.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(j))))))(chanx,:,:,1:NTrials_ERSP(j)) = PLOT.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(j))))));
            ...
            if sum(strcmp(PLOT.types,strcat(PHASEcond(PHASEcLength),'-line'))>0)
                ...
                if NumConds==1 % 95% confidence interval
                    ...
                    plotSize = size(PLOT.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(1)))))));
...
%                         tempPerm = repmat(tempPerm,plotSize(1));
                        ...
                        signx = reshape(signx,[1 1 plotSize(3)]);
                        signx = repmat(signx,plotSize(1),plotSize(2));
                        tempPerm = wrapToPi(PLOT.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(1))))))+signx);
                ...
                elseif NumConds==2 % ttest
                    ...
                    tempdata1 = PLOT.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(1))))));
                    tempdata2 = PLOT.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(2))))));
                ...
                elseif NumConds==3 % ANOVA
                    ...
                    tempdata1 = PLOT.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(1))))));
                    tempdata2 = PLOT.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(2))))));
                    tempdata3 = PLOT.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(3))))));
                ...
                elseif NumConds==4 % ANOVA
                    ...
                    tempdata2 = PLOT.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(2))))));
                    tempdata3 = PLOT.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(3))))));
                    tempdata4 = PLOT.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(4))))));
                    tempMean1 = AVG.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(1)),'_avg'))))(chanx,:);
                    tempMean2 = AVG.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(2)),'_avg'))))(chanx,:);
                    tempMean3 = AVG.(genvarname(char(strcat(PHASEcond(PHASEcLength),num2str(plot_conds(3)),'_avg'))))(chanx,:);
    ...
    while analysescounter <= length(PLOT.types)
        ...
        if sum(strcmp(PLOT.types(analysescounter),'ERP'))>0
            ...
            if ismember(NumConds,1:2)
                h(1) = plot(AVG.(genvarname(char(strcat('ERP',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(1)),'LineWidth',LineThickness);
                hold on
                ...
                if NumConds>1
                    j=2;
                    h(j) = plot(AVG.(genvarname(char(strcat('ERP',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(j)),'LineWidth',LineThickness);
        ...
        if sum(strcmp(PLOT.types(analysescounter),'ERSP-line'))>0
            ...
            if ismember(NumConds,1:2)
                h(1) = plot(AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(1)),'LineWidth',LineThickness);
                hold on
                ...
                if NumConds>1 
                    j=2;
                    h(j) = plot(AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(j)),'LineWidth',LineThickness);
        ...
        if sum(strcmp(PLOT.types(analysescounter),'ERSP-individ'))>0
            ...
            hold on
            for j=1:NumConds
                h(j) = plot(squeeze(AVG.(genvarname(char(strcat('ERSP',num2str(plot_conds(j)),'_indiv'))))(chanx,xaxis,1)),'Color',char(colorarray(j)),'LineWidth',LineThickness);
            end
            legend(h,legend_array,'Location','NorthWest');
            for j=1:NumConds
        ...
        if sum(strcmp(PLOT.types(analysescounter),'Induced-line'))>0
            subplot(PLOT.rowcount,PLOT.colcount,PLOT.plotlocs(analysescounter,:))
            if ismember(NumConds,1:2)
                h(1) = plot(AVG.(genvarname(char(strcat('Induced',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(1)),'LineWidth',LineThickness);
                hold on
                ...
                if NumConds>1 
                    j=2;
                    h(j) = plot(AVG.(genvarname(char(strcat('Induced',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(j)),'LineWidth',LineThickness);
        ...
        if sum(strcmp(PLOT.types(analysescounter),'ITPC-line'))>0
            ...
            if ismember(NumConds,1:2)
                h(1) = plot(AVG.(genvarname(char(strcat('ITPC',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(1)),'LineWidth',LineThickness);
                hold on
                errbar = AVG.ITPCstats(chanx,xaxis);
                shadedErrorBar(1:length(xaxis),AVG.(genvarname(char(strcat('ITPC',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis),errbar,colorarray(1),.5);
                if NumConds>1 
                    j=2;
                    h(j) = plot(AVG.(genvarname(char(strcat('ITPC',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(j)),'LineWidth',LineThickness);
                    shadedErrorBar(1:length(xaxis),AVG.(genvarname(char(strcat('ITPC',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),errbar,colorarray(j),.5);
                end
            end
            ...
            if ismember(NumConds,3:4)
                ...
                for j=1:NumConds
                    h(j) = plot(AVG.(genvarname(char(strcat('ITPC',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(j)),'LineWidth',LineThickness);
                    shadedErrorBar(1:length(xaxis),AVG.(genvarname(char(strcat('ITPC',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),AVG.(genvarname(char(strcat('ITPC',num2str(plot_conds(j)),'_err'))))(chanx,xaxis),colorarray(j),.5);
                end
            end
        ...
        if sum(strcmp(PLOT.types(analysescounter),'R2-line'))>0
            subplot(PLOT.rowcount,PLOT.colcount,PLOT.plotlocs(analysescounter,:))
            if ismember(NumConds,1:2)
                h(1) = plot(AVG.(genvarname(char(strcat('R2',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(1)),'LineWidth',LineThickness);
                hold on
                errbar = AVG.R2stats(chanx,xaxis);
                shadedErrorBar(1:length(xaxis),AVG.(genvarname(char(strcat('R2',num2str(plot_conds(1)),'_avg'))))(chanx,xaxis),errbar,colorarray(1),.5);
                if NumConds>1 
                    j=2;
                    h(j) = plot(AVG.(genvarname(char(strcat('R2',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(j)),'LineWidth',LineThickness);
                    shadedErrorBar(1:length(xaxis),AVG.(genvarname(char(strcat('R2',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),errbar,colorarray(j),.5);
                end
            end
            if ismember(NumConds,[3:4])
                tempmin = [];
                ...
                for j=1:NumConds
                    h(j) = plot(AVG.(genvarname(char(strcat('R2',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),'Color',char(colorarray(j)),'LineWidth',LineThickness);
                    shadedErrorBar(1:length(xaxis),AVG.(genvarname(char(strcat('R2',num2str(plot_conds(j)),'_avg'))))(chanx,xaxis),AVG.(genvarname(char(strcat('R2',num2str(plot_conds(j)),'_err'))))(chanx,xaxis),colorarray(j),.5);
                end
            end
