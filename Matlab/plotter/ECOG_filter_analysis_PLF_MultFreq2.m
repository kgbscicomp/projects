...
hband_sig_data=zeros([size(data,1) 1]);
...
for el=1:size(data,2)
    ...
    for yy = 1:size(hband_filters,1)
        ...
        % Butterworth Filters
        hband_sig_data = butterpass_eeglabdata(data(:,el),hband,srate,ButterRp,ButterRs);%3,60);
        hband_sig_data = hilbert(hband_sig_data);
        hband_buttord_wide(:,yy,el)=hband_sig_data;
...
for elec=1:num_el
    ...
    for yyy=1:numevents
        ...
        eeglab_sig_wide(yyy,:,elec)=...
            squeeze(data(currsample+(epoch_adj(1)*srate_new):currsample+(epoch_adj(2)*srate_new),elec))';
    end
end
